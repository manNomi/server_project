...
function newPuzzleFinish() {
  var children = document.getElementById("first_page").children;
...
function nextPageEvent() {
  ...
  // second_page_old의 자식 노드를 second_page_rotate에 추가
  for (var i = 0; i < second_page_old.children.length; i++) {
    var child = second_page_old.children[i];
    // canvas 요소일 경우 내용을 복사
    // === -> 엄격하게 같은지
    // 캔버스에 이미지 요소가 복사가 안되기에 context를 통해 그려주는 방식
    if (child.tagName === "CANVAS") {
      var newCanvas = document.createElement("canvas");
      newCanvas.width = child.width; // 원본 canvas의 너비 복사
      newCanvas.height = child.height; // 원본 canvas의 높이 복사
      newCanvas.classList.add("puzzle_img"); // 태그 크기
      var context = newCanvas.getContext("2d");
      context.drawImage(child, 0, 0);
      second_page_rotate.appendChild(newCanvas);
    } else {
      // 다른 요소는 그대로 복사
      var newChild = child.cloneNode(true);
      newChild.classList.add("puzzle_img");
      second_page_rotate.appendChild(newChild);
    }
  }
  ...
  // 넘어가는 타이밍 맞춰서 삭제
  setTimeout(() => {
    second_page_rotate.remove();
  }, 550);
  // 넘어가는 시간 맞춰서 사라지게끔 하기
  setTimeout(() => {
    if (first_click_event == true) {
      first_page_old.remove();
      second_page_old.remove;
    } else {
      document
        .getElementById("main_book_cover_back_container")
        .replaceChildren();
...
function newPuzzleImgMake(cut_img_list) {
  for (i = 0; i < 9; i++) {
    ...
    tmpImgTag.src = "../image/puzzle/empty.png";
...
function puzzleImgMake(name, fist, second) {
  ...
  var imgList = [];
  for (i = 1; i < 10; i++) {
    var imgSrc = "../image/puzzle//" + name + "_" + i + ".webp";
    imgList.push(imgSrc);
  }
